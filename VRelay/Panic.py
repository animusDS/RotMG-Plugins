...
class Panic(PluginInterface):
    ...
    unsafeDungeons = ['Garden of Chaos',
                      'Crypt of the Illusionist', 'Trial of the Illusionist']
    ...
    def onHello(self, client: Client, packet: Hello, send: bool) -> (Hello, bool):
        if self.createNotif == True:
            client.createNotification(
                "Panic Mode", "Nexused due to player joining you in an unsafe dungeon. Be more carful next time!")
    ...
    def onPlayerText(self, client: Client, packet: PlayerText, send: bool) -> (PlayerText, bool):
        if packet.text[0:7] == "/panic ":
            try:
                ...
                client.createNotification(
                    "Panic Mode", "Set Panic Mode to {}".format(self.panicMode))
            except:
                client.createNotification(
                    "Panic Mode", "Incorrect syntax. Use '/panic ON | OFF'.")
            send = False
        return (packet, send)
