...
class UnderageGambling(PluginInterface):
    ...
    hooks = {PacketTypes.Update, PacketTypes.PlayerText,
             PacketTypes.Hello, PacketTypes.Text}
    load = True
    defaultState = True
                ...
                if d == p.objectStatusData.objectID:
                    ...
                    if self.debug:
                        for s in p.objectStatusData.stats:
                            if s.statType == 31:
                                print("Removed {} from list".format(
                                    s.strStatValue))
        if self.debug:
        ...
        if packet.text[0:7] == "/debug ":
            try:
                ...
                client.createNotification(
                    self.pluginName, "Set Debug Mode to {}".format(self.debug))
            except:
                client.createNotification(
                    self.pluginName, "Incorrect syntax. Use '/test ON | OFF'.")
            send = False

        if packet.text[0:8] == "/target ":
            self.target = packet.text.split(" ")[-1]
            client.createNotification(
                self.pluginName, "Set Targete to {}".format(self.target))
            send = False

        if packet.text[0:8] == "/scount ":
            self.spamCount = int(packet.text.split(" ")[-1])
            client.createNotification(
                self.pluginName, "Set count to {}".format(self.spamCount))
            send = False

        if packet.text[0:12] == "/sleepcount ":
            self.sleepCount = float(packet.text.split(" ")[-1])
            client.createNotification(
                self.pluginName, "Set count to {}".format(self.sleepCount))
            send = False
        ...
        if packet.text[0:8] == "/gamble ":
            ...
            if packet.text.split(" ")[1] == "spam":
                ...
                if name == "all":
                    ...
                    client.createNotification(
                        self.pluginName, "Sent gamble request to {} players.".format(len(self.players)))
                else:
                    g.name = name
                    for i in range(self.spamCount):
                        client.SendPacketToServer(CreatePacket(g))
                        time.sleep(self.sleepCount)
                client.createNotification(
                    self.pluginName, "Sent gamble request to {} {} times!".format(name, i+1))
                ...
                g.amount = amount
                client.createNotification(
                    self.pluginName, "Sent gamble request to {} for {}.".format(g.name, g.amount))
                client.SendPacketToServer(CreatePacket(g))
